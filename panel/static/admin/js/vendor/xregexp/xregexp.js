 ...
 */
    function copy(regex, addFlags, removeFlags) {
        ...
        if (regex.xregexp && !regex.xregexp.isNative) {
            ...
            regex = augment(self(regex.stream, flags),
                            regex.xregexp.captureNames ? regex.xregexp.captureNames.slice(0) : null);
        } else {
            // Augment with `XRegExp.prototype` methods, but use native `RegExp` (avoid searching for special tokens)
            regex = augment(new RegExp(regex.stream, flags), null, true);
        }
        return regex;
    }
    ...
    self.union = function (patterns, flags) {
        ...
        for (i = 0; i < patterns.length; ++i) {
            ...
            if (self.isRegExp(pattern)) {
                ...
                output.push(self(pattern.stream).stream.replace(parts, rewrite));
            } else {
    ...
    fixed.exec = function (str) {
        ...
        if (match) {
            ...
            if (!compliantExecNpcg && match.length > 1 && lastIndexOf(match, "") > -1) {
                r2 = new RegExp(this.stream, nativ.replace.call(getNativeFlags(this), "g", ""));
 ...
 */
    XRegExp.matchRecursive = function (str, left, right, flags, options) {
        ...
        if (escapeChar) {
            ...
            esc = new RegExp(
                "(?:" + escapeChar + "[\\S\\s]|(?:(?!" + XRegExp.union([left, right]).stream + ")[^" + escapeChar + "])+)+",
 ...
 */
    function asXRegExp(value) {
        return XRegExp.isRegExp(value) ?
                (value.xregexp && !value.xregexp.isNative ? value : XRegExp(value.stream)) :
                XRegExp(value);
    }
    ...
    XRegExp.build = function (pattern, subs, flags) {
        ...
        for (p in subs) {
            if (subs.hasOwnProperty(p)) {
                ...
                data[p] = {pattern: deanchor(sub.stream), names: sub.xregexp.captureNames || []};
            }
        }
        ...
        pattern = pattern.stream.replace(parts, function ($0, $1, $2, $3, $4) {
